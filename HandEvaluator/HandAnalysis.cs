...
namespace HoldemHand
{
    public partial class Hand : IComparable
    {
        ...
        #region "Global Vars For Distributed HP"
        
            private struct innerInfo
            {
                public ulong board;
                public ulong pocket;
                public ulong oppPocket;
                public int index;
            };

            private static int[,] HP_global = new int[3, 3];
            private static int total_global = 0;
        ...
        /// <returns>Hand strength as a percentage of hands won.</returns>
        public static double HandStrength(ulong pocket, ulong board, ulong dead )
        {
            double win = 0.0, count = 0.0;
#if DEBUG
            if (BitCount(pocket) != 2)
...
#endif
            uint ourrank = Evaluate(pocket | board);
            foreach (ulong oppcards in Hand.Hands(0UL, pocket | board | dead, 2))
            {
                uint opprank = Evaluate(oppcards | board);
                if (ourrank > opprank)
                {
        ...
        /// <summary>
        /// This function returns true max number of suits
        /// </summary>
        /// <param name="mask">mask to check for "suited-ness"</param>
        /// <returns>max number of suits.</returns>
        public static int suitCount(ulong mask)
        {
            uint sc = CardMask(mask, Clubs);
            uint sd = CardMask(mask, Diamonds);
            uint sh = CardMask(mask, Hearts);
            uint ss = CardMask(mask, Spades);
            return Math.Max(Math.Max(Math.Max(BitCount(sc), BitCount(sd)), BitCount(sh)), BitCount(ss));
        }
        ...
        static public uint rankedNess(ulong cards, int nCards)
        {
            // Seperate out by suit
        ...
        #region isPair
        /// <summary>
        /// True if card is a pair
        /// </summary>
        /// <param name="mask">two card mask mask</param>
        /// <returns>number of spaces between two cards</returns>
        static public bool isPair(ulong mask)
        {
            int start, end;
       ...
       /// <returns>True means approximate results</returns>
        ...
        private static void innerCrunch(Object data)
        {
            const int ahead = 2;
            const int tied = 1;
            const int behind = 0;
            ...
            innerInfo io = (innerInfo)data;

            foreach (ulong boardmask in Hands(io.board, io.pocket | io.oppPocket, 5))
            {
                ourbest = Evaluate(io.pocket | boardmask, 7);
                oppbest = Evaluate(io.oppPocket | boardmask, 7);
                if (ourbest > oppbest)
                    HP_global[io.index, ahead]++;
                else if (ourbest == oppbest)
                    HP_global[io.index, tied]++;
                else
                    HP_global[io.index, behind]++;
            }

            lock (c_lock) { total_global++; }
        ...
        /// <param name="ppot">Positive Potential</param>
        /// <param name="npot">Negative Potential</param>
        public static void HandPotential_Dist(ulong pocket, ulong board, out double ppot, out double npot)
        {
            const int ahead = 2;
            const int tied = 1;
            const int behind = 0;
            ...
            foreach (ulong oppPocket in Hands(0UL, pocket | board, 2))
            {
                ...
                innerInfo io = new innerInfo();

                io.pocket = pocket;
                io.board = board;
                io.oppPocket = oppPocket;
                io.index = index;

                System.Threading.ThreadPool.QueueUserWorkItem(new System.Threading.WaitCallback(innerCrunch), io);

                HPTotal[index]++;
                total++;
            }

            while (total_global < total)
                System.Threading.Thread.Sleep(10);

            double den1 = (mult * (HPTotal[behind] + (HPTotal[tied] / 2.0)));
            double den2 = (mult * (HPTotal[ahead] + (HPTotal[tied] / 2.0)));
            if (den1 > 0)
                ppot = (HP_global[behind, ahead] + (HP_global[behind, tied] / 2) + (HP_global[tied, ahead] / 2)) / den1;
            else
                ppot = 0;
            if (den2 > 0)
                npot = (HP_global[ahead, behind] + (HP_global[ahead, tied] / 2) + (HP_global[tied, behind] / 2)) / den2;
            else
                npot = 0;
        }
        ...
        /// <param name="ppot">Positive Potential</param>
        /// <param name="npot">Negative Potential</param>
        public static void HandPotential(ulong pocket, ulong board, ulong dead, out double ppot, out double npot)
        {
            const int ahead = 2;
            const int tied = 1;
            const int behind = 0;
            ...
            // Iterate through all possible opponent pocket cards
            foreach (ulong oppPocket in Hands(0UL, pocket | board | dead, 2))
            {
                uint opprank = Evaluate(oppPocket | board, ncards);
                int index = (ourrank > opprank ? ahead : (ourrank == opprank ? tied : behind));
                foreach (ulong boardmask in Hands(board, pocket | oppPocket | dead, 5))
                {
                    ourbest = Evaluate(pocket | boardmask, 7);
                    oppbest = Evaluate(oppPocket | boardmask, 7);
                    if (ourbest > oppbest)
